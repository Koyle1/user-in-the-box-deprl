...
class SAC(BaseRLModel):
  ...
  def __init__(self, simulator, checkpoint_path=None, wandb_id=None, info_keywords=()):
    ...
    if checkpoint_path is not None:
        # Resume training
        self.model = SAC_sb3_customlogs.load(checkpoint_path, parallel_envs, verbose=1, #policy_kwargs=rl_config["policy_kwargs"], 
        ...
        # Initialise model
        self.model = SAC_sb3_customlogs(rl_config["policy_type"], parallel_envs, verbose=1, policy_kwargs=rl_config["policy_kwargs"],
                             tensorboard_log=simulator_folder, n_steps=rl_config["nsteps"],
                             batch_size=rl_config["batch_size"], target_kl=rl_config["target_kl"],
                             learning_rate=rl_config["lr"], device=rl_config["device"])
...
class SAC_sb3_customlogs(SAC_sb3):
   '''
   ...
   def __init__(self, *args, **kwargs):
       ...
       filtered_kwargs = {k: v for k, v in kwargs.items() if k not in not_valid_keys} 
