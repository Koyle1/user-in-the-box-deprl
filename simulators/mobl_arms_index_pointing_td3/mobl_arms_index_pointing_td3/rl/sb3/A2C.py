...
class A2C(BaseRLModel):
  ...
  def __init__(self, simulator, checkpoint_path=None, wandb_id=None, info_keywords=()):
    ...
    if checkpoint_path is not None:
        # Resume training
        self.model = A2C_sb3_customlogs.load(checkpoint_path, parallel_envs, verbose=1, #policy_kwargs=rl_config["policy_kwargs"], 
        ...
        # Initialise model
        self.model = A2C_sb3_customlogs(rl_config["policy_type"], parallel_envs, verbose=1, policy_kwargs=rl_config["policy_kwargs"],
                             tensorboard_log=simulator_folder, n_steps=rl_config["nsteps"],
                             batch_size=rl_config["batch_size"], target_kl=rl_config["target_kl"],
                             learning_rate=rl_config["lr"], device=rl_config["device"])
...
class A2C_sb3_customlogs(A2C_sb3):
   '''
   def __init__(self, *args, **kwargs):
        # Optional: filter kwargs here, as shown earlier
        valid_params = set(signature(A2C.__init__).parameters.keys())
        filtered_kwargs = {k: v for k, v in kwargs.items() if k in valid_params}

        discarded_keys = [k for k in kwargs if k not in valid_params]
        if discarded_keys:
            print(f"Warning: Ignoring unsupported A2C arguments: {discarded_keys}")

        # Pass filtered kwargs and args to super
        super().__init__(*args, **filtered_kwargs)
    '''
   def __init__(self, *args, **kwargs):
        # Set of known valid A2C keyword arguments (simplified SB3 2.2.1+ version)
       valid_keys = {
            "policy", "env", "learning_rate", "n_steps", "gamma", "gae_lambda", "ent_coef",
            "vf_coef", "max_grad_norm", "use_rms_prop", "rms_prop_eps", "use_sde",
            "sde_sample_freq", "tensorboard_log", "policy_kwargs", "verbose", "seed",
            "device", "_init_setup_model"
       }
       filtered_kwargs = {k: v for k, v in kwargs.items() if k in valid_keys}
       discarded = set(kwargs.keys()) - valid_keys
       if discarded:
           print(f"Warning: Ignoring unsupported A2C arguments: {discarded}")
